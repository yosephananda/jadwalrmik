<div>
    <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
        <div>
            <h4 class="mb-3 mb-md-0">Mata Kuliah</h4>
        </div>
    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="dataTableExample" class="table">
                            ...
                            <tbody>
                                @foreach ($matakul as $mtk)
                                    <tr>
                                        <td>{{ $mtk->kd_matkul }}</td>
                                        <td>{{ $mtk->nama_matkul }}</td>
                                        <td><a class="btn btn-outline-success"
                                                wire:click.prevent="viewRubah({{ $mtk->id }})">
                                                <i data-feather="edit"></i></a>
                                            <a class="btn btn-outline-danger"
                                                wire:click.prevent="viewHapus({{ $mtk->id }})">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">
                        @if ($showEdit)
                            Ubah Informasi Mata Kuliah
                        @else
                            Tambah Informasi Mata Kuliah
                        @endif
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="btn-close"></button>
                </div>
                <form wire:submit.prevent="{{ $showEdit ? 'updateMatkul' : 'createMatkul' }}" autocomplete="off">
                    <div class="modal-body">

                        <div class="row mb-3">
                            <label for="kd_matkul" class="col-sm-3 col-form-label">Kode Mata Kuliah: </label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="kd_matkul"
                                    wire:model.defer="data.kd_matkul">
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="nama_matkul"
                                    wire:model.defer="data.nama_matkul">
                            </div>
                        </div>
                    </div>
